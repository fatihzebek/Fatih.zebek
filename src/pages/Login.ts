      ...
      <div class="login-glass-panel">
        ...
        <form id="login-form">
          <div class="form-group" style="margin-bottom: 1.5rem;">
            <label style="color: rgba(255,255,255,0.7); font-size: 0.65rem; letter-spacing: 1.5px; font-weight: 800;"><i class="fa-solid fa-user" style="color: #64ffda; margin-right: 5px;"></i> E-POSTA ADRESİ VEYA KULLANICI ADI</label>
            <input type="text" id="email" class="cyber-input" style="background: rgba(0,0,0,0.5); border: 1px solid rgba(100, 255, 218, 0.15); box-shadow: inset 0 2px 4px rgba(0,0,0,0.5); padding: 1rem;" placeholder="admin@demirerholding.com veya kullanıcı adı" required autocomplete="off">
          </div>
...
(window as any).handleLogin = async (e: Event) => {
  ...
  const errorDiv = document.getElementById('login-error');
  const submitBtn = e.target ? (e.target as HTMLFormElement).querySelector('button') : (document.querySelector('#login-form button') as HTMLButtonElement);

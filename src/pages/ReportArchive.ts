...
export const ReportArchivePage = async (siteId?: string) => {
  try {
      ...
      return `
        <div class="fade-in-up content-area">
          <div class="glass-panel" style="padding: 3rem; text-align: center;">
            <i class="fa-solid fa-folder-tree" style="font-size: 4rem; color: var(--accent-blue); margin-bottom: 1.5rem; opacity: 0.3;"></i>
            <h2 style="margin-bottom: 1rem;">Rapor Arşivi</h2>
            <p style="color: var(--text-muted);">Lütfen soldaki menüden raporlarını görmek istediğiniz sahayı seçiniz.</p>
          </div>
        </div>
      `;
    }
      ...
      <div class="glass-panel" style="padding: 1.5rem;">
        ${visibleReports.length === 0 ? `
          <div style="padding: 5rem; text-align: center; color: var(--text-muted);">
            ...
            <p>Bu sahaya ait görüntülenecek rapor bulunamadı.</p>
          </div>
        ` : `
            <div style="overflow-x: auto;">
              ...
              <tbody>
                ${visibleReports.map(report => `
                  <tr class="archive-row" data-date="${report.date}" style="border-bottom: 1px solid rgba(255,255,255,0.03); transition: background 0.3s;">
                    ...
                    <td style="padding: 1rem;">
                      <div style="display: flex; flex-direction: column; gap: 4px;">
                        ...
                        <span style="font-size: 0.65rem; color: var(--text-muted); max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600; text-transform: uppercase;">
                          ${report.templateName ? (report.faultDesc || '') : (report.faultDesc || '')}
                        </span>
                      </div>
                    </td>
                    <td style="padding: 1rem;">
                    ...
                    <td style="padding: 1rem; text-align: right; white-space: nowrap;">
                      <div style="display: flex; gap: 0.5rem; justify-content: flex-end; align-items: center;">
                        ...
                        ` : ''}
                        ${canReturn ? `
                        <button class="btn-cyber" title="Ekibe Geri Gönder" style="padding: 6px 10px; font-size: 0.7rem; background: rgba(155, 89, 182, 0.1); border: 1px solid #9b59b2; color: #9b59b2;" onclick="window.sendReportBackToTeam('${report.id}', '${report.reportNo}', '${report.siteId}')">
                          <i class="fa-solid fa-reply"></i>
                        </button>
                        ` : ''}
                        ${canDelete ? `
                        <button class="btn-cyber" title="Sil" style="padding: 6px 10px; font-size: 0.7rem; background: rgba(255, 0, 85, 0.1); border: 1px solid #ff0055; color: #ff0055;" onclick="window.deleteArchiveReport('${report.id}', '${report.reportNo}', '${report.siteId}')">
    ...
    <div id="report-modal" class="hidden" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #050a10; z-index: 9999; display: flex; flex-direction: column; animation: fadeIn 0.3s ease;">
      <!-- Premium Preview Header -->
      <div style="background: rgba(255, 255, 255, 0.03); border-bottom: 1px solid rgba(255, 255, 255, 0.05); padding: 1rem 2rem; display: flex; justify-content: space-between; align-items: center; backdrop-filter: blur(20px);">
      ...
      <div id="report-modal-content" style="flex: 1; padding: 2rem 1rem; background: radial-gradient(circle at center, rgba(0, 242, 254, 0.03) 0%, transparent 70%); overflow-y: auto; overflow-x: auto; scroll-behavior: smooth;"></div>
    </div>
...
(window as any).viewArchiveReport = async (reportNo: string) => {
    try {
        ...
        if (report) {
            ...
            const previewReportNo = document.getElementById('preview-report-no');
            ...
            if (modal && modalContent) {
                ...
                if (isAdmin) {
                    if (report.auditMetrics) {
                        ...
                        telemetryHtml = `
                        <div class="no-print" style="background: linear-gradient(135deg, rgba(15, 23, 42, 0.95), rgba(30, 41, 59, 0.95)); border: 1px solid ${isSuspicious ? 'rgba(255, 0, 85, 0.3)' : 'rgba(0, 242, 254, 0.3)'}; margin: 20px auto; max-width: 900px; border-radius: 12px; padding: 1.25rem; color: #fff; box-shadow: 0 10px 30px rgba(0,0,0,0.5); display: flex; flex-direction: column; gap: 0.75rem; font-family: 'Inter', sans-serif;">
                          <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 0.5rem;">
                            <div style="display: flex; align-items: center; gap: 10px;">
                              <i class="fa-solid ${isSuspicious ? 'fa-triangle-exclamation' : 'fa-circle-check'}" style="font-size: 1.3rem; color: ${isSuspicious ? '#ff0055' : '#00f2fe'};"></i>
                              <div>
                                <h4 style="margin: 0; font-family: 'Rajdhani', sans-serif; font-size: 1.05rem; font-weight: 800; letter-spacing: 0.5px; text-transform: uppercase;">CHECKLIST DOĞRULUK ANALİZİ (ADMİN TELEMETRİSİ)</h4>
                                <p style="margin: 2px 0 0; font-size: 0.65rem; color: var(--text-muted); font-weight: 600;">Bu panel sadece yöneticilere gösterilir. PDF çıktısında yer almaz.</p>
                              </div>
                            </div>
                            <span style="background: ${isSuspicious ? 'rgba(255, 0, 85, 0.15)' : 'rgba(0, 242, 254, 0.15)'}; color: ${isSuspicious ? '#ff0055' : '#00f2fe'}; border: 1px solid ${isSuspicious ? '#ff0055' : '#00f2fe'}; padding: 3px 10px; border-radius: 6px; font-weight: 800; font-size: 0.7rem; letter-spacing: 0.5px; display: flex; align-items: center; gap: 4px;">
                              ${isSuspicious ? '⚠️ ŞÜPHELİ / HIZLI DOLDURMA' : '🟢 GÜVENİLİR DURUM'}
                            </span>
                          </div>
                          ...
                          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 0.75rem;">
                            <div style="background: rgba(255,255,255,0.02); padding: 8px 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.04);">
                              <div style="font-size: 0.6rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; margin-bottom: 2px;">Toplam Süre</div>
                              <div style="font-size: 1rem; font-weight: 800; font-family: 'Rajdhani', sans-serif; color: #fff;">${metrics.totalFillTimeSeconds || 0} saniye</div>
                              <div style="font-size: 0.55rem; color: var(--text-muted); font-weight: 600;">(Form açık kalma süresi)</div>
                            </div>
                            
                            <div style="background: rgba(255,255,255,0.02); padding: 8px 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.04);">
                              <div style="font-size: 0.6rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; margin-bottom: 2px;">Toplam İşaretleme</div>
                              <div style="font-size: 1rem; font-weight: 800; font-family: 'Rajdhani', sans-serif; color: #fff;">${metrics.clickCount || 0} tıklama</div>
                              <div style="font-size: 0.55rem; color: var(--text-muted); font-weight: 600;">(Seçim güncellemeleri)</div>
                            </div>
                            
                            <div style="background: rgba(255,255,255,0.02); padding: 8px 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.04);">
                              <div style="font-size: 0.6rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; margin-bottom: 2px;">Ortalama Tıklama Hızı</div>
                              <div style="font-size: 1rem; font-weight: 800; font-family: 'Rajdhani', sans-serif; color: ${metrics.averageClickIntervalMs < 1800 ? '#ff0055' : '#00f2fe'};">
                                ${metrics.averageClickIntervalMs ? (metrics.averageClickIntervalMs / 1000).toFixed(1) : 0} sn
                              </div>
                              <div style="font-size: 0.55rem; color: var(--text-muted); font-weight: 600;">(Tıklamalar arası ortalama)</div>
                            </div>
                            
                            <div style="background: rgba(255,255,255,0.02); padding: 8px 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.04);">
                              <div style="font-size: 0.6rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; margin-bottom: 2px;">Ardışık Hızlı Seçim</div>
                              <div style="font-size: 1rem; font-weight: 800; font-family: 'Rajdhani', sans-serif; color: ${metrics.maxConsecutiveFastSameStatus >= 8 ? '#ff0055' : '#fff'};">
                                ${metrics.maxConsecutiveFastSameStatus || 0} madde
                              </div>
                              <div style="font-size: 0.55rem; color: var(--text-muted); font-weight: 600;">(Aynı durumla ardışık < 2.5s)</div>
                            </div>
                          </div>
                          
                          ${isSuspicious && metrics.suspicionReason ? `
                          <div style="background: rgba(255, 0, 85, 0.08); border: 1px dashed rgba(255, 0, 85, 0.3); padding: 10px; border-radius: 8px; font-size: 0.75rem; color: #ffb3c6; display: flex; gap: 8px; align-items: flex-start; margin-top: 4px;">
                            <i class="fa-solid fa-circle-info" style="margin-top: 1px; color: #ff0055;"></i>
                            <div>
                              <span style="font-weight: 700;">Şüphe Nedeni:</span> ${metrics.suspicionReason}
                            </div>
                          </div>
                          ` : ''}
                        </div>
                        ...
                        telemetryHtml = `
                        <div class="no-print" style="background: linear-gradient(135deg, rgba(15, 23, 42, 0.95), rgba(30, 41, 59, 0.95)); border: 1px solid rgba(255, 255, 255, 0.15); margin: 20px auto; max-width: 900px; border-radius: 12px; padding: 1rem 1.25rem; color: #fff; box-shadow: 0 10px 30px rgba(0,0,0,0.5); display: flex; align-items: center; gap: 12px; font-family: 'Inter', sans-serif;">
                          <i class="fa-clock-rotate-left fa-solid" style="font-size: 1.2rem; color: var(--text-muted);"></i>
                          <div>
                            <h4 style="margin: 0; font-family: 'Rajdhani', sans-serif; font-size: 0.95rem; font-weight: 800; color: var(--text-muted); text-transform: uppercase;">ANALİZ VERİSİ BULUNMUYOR (ESKİ RAPOR)</h4>
                            <p style="margin: 2px 0 0; font-size: 0.65rem; color: var(--text-muted);">Bu rapor analiz sisteminin aktif edilmesinden önce oluşturulduğu için doğrululuk analizi verileri bulunmamaktadır.</p>
                          </div>
                        </div>
                        `;
                    }
...
(window as any).downloadReportPDF = async (event: Event) => {
    ...
    try {
        ...
        if (report) {
            ...
            // html2pdf.js kütüphanesini yükle
            if (!(window as any).html2pdf) {
                await new Promise((res, rej) => {
                    const script = document.createElement('script');
                    script.src = 'https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js';
                    ...
                    <html>
                        <head>
                            ...
                            <style>
                                ...
                                html { font-size: 12px; } /* Varsayılan 16px olan font boyutunu %25 küçülttük, rem değerleri buna göre küçülür */
                                body { 
                                ...
                                /* PDF kaydetme yönlendirmesi - yazdırma sırasında gizlenir */
                                .pdf-guide {
                                    background: #1e293b;
                                    color: #fff;
                                    padding: 12px 20px;
                                ...
                                .pdf-guide span { color: #00f2fe; font-weight: 800; }
                                @media print {
                                    .pdf-guide { display: none !important; }
                                }
                            </style>
                        </head>
                        <body>
                            <div class="pdf-guide">
                                📋 Kusursuz Görünüm İçin: Açılan pencerede <span>"Hedef/Yazıcı"</span> kısmından <span>"PDF olarak kaydet"</span> seçeneğini seçin.
                            </div>
                            ${htmlContent}
                            <script>
                                window.onload = () => {

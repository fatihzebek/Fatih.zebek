...
const renderCircularGauge = (percentage: number, colorStart: string, colorEnd: string, id: string) => {
  const radius = 38;
  const strokeWidth = 7;
  ...
  return `
    <div class="circular-gauge-wrapper" style="position: relative; width: 80px; height: 80px; flex-shrink: 0;">
      <svg width="80" height="80" viewBox="0 0 100 100" style="transform: rotate(-90deg); display: block;">
        <defs>
        ...
        <!-- Background circle -->
        <circle cx="50" cy="50" r="${radius}" fill="transparent" stroke="rgba(255, 255, 255, 0.03)" stroke-width="${strokeWidth}"></circle>
        <!-- Progress circle -->
        <circle cx="50" cy="50" r="${radius}" fill="transparent" stroke="url(#grad-${id})" stroke-width="${strokeWidth}" 
          stroke-dasharray="${circumference}" 
          stroke-dashoffset="${offset}" 
          stroke-linecap="round"
          style="transition: stroke-dashoffset 1.5s cubic-bezier(0.4, 0, 0.2, 1); filter: drop-shadow(0 0 4px ${colorStart}40);">
        </circle>
      </svg>
      <!-- Center text -->
      <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; font-family: 'Rajdhani', sans-serif; font-weight: 800; font-size: 1.1rem; color: var(--text-main);">
        ${percentage}%
      </div>
    </div>
  `;
...
export const DashboardPage = async () => {
  ...
  // 1. Completion Rate (solved tasks vs total tasks today)
  const completedToday = tasks.filter(t => t.status === 'Tamamlandı').length;
  const totalTasksToday = completedToday + activeTasksCount;
  const completionRate = totalTasksToday > 0 ? Math.round((completedToday / totalTasksToday) * 100) : 100;

  // 2. Active Fault Rate (emergency tasks out of total active tasks)
  const faultRate = activeTasksCount > 0 ? Math.round((emergencyTasksCount / activeTasksCount) * 100) : 0;

  // 3. Turbine Health Rate
  let totalTurbines = 0;
  let healthyTurbines = 0;
  sites.forEach(site => {
    const siteTurbines = dataService.getTurbinesBySite(site.id);
    totalTurbines += siteTurbines.length;
    siteTurbines.forEach(t => {
      const hasActiveFault = openTasks.some(ot => ot.turbineId === t.no.toString() && ot.siteId === site.id && ot.secilenSablon?.toLowerCase().includes('arıza'));
      if (!hasActiveFault) {
        healthyTurbines++;
      }
    });
  });
  const healthRate = totalTurbines > 0 ? Math.round((healthyTurbines / totalTurbines) * 100) : 100;

  // 4. Critical Maintenance Ratio
  const overdueMaintCount = maintenancePlan.filter(p => p.status === 'overdue').length;
  const warningMaintCount = maintenancePlan.filter(p => p.status === 'warning').length;
  const criticalMaintCount = overdueMaintCount + warningMaintCount;
  const maintenanceRatio = totalTurbines > 0 ? Math.round((criticalMaintCount / totalTurbines) * 100) : 0;

  // Real-time Agent Monitoring + Counter Animation + Agenda + Wind Speed Canvas
  setTimeout(() => {
    ...
    // ===== WIND SPEED CANVAS GRAPH =====
    const canvas = document.getElementById('wind-speed-canvas') as HTMLCanvasElement;
    if (canvas) {
      ...
      if (ctx) {
        ...
        const windData = Array.from({length: pointsCount}, (_, i) => {
          return {
            x: i,
            y: 6 + Math.sin(i * 0.7) * 3 + Math.cos(i * 1.3) * 1.5 + Math.random() * 0.5
          };
        });
        ...
        const animate = () => {
          // If element is not in DOM anymore, stop loop
          const canvasEl = document.getElementById('wind-speed-canvas');
          if (!canvasEl) {
          ...
          // Draw horizontal average speed line
          ctx.strokeStyle = 'rgba(0, 243, 255, 0.15)';
          ctx.setLineDash([5, 5]);
          ctx.beginPath();
          const avgY = h - (8.2 / 18) * h;
          ctx.moveTo(0, avgY);
          ctx.lineTo(w, avgY);
          ctx.stroke();
          ctx.setLineDash([]);
          ...
          ctx.fillText('ORT: 8.2 m/s', 10, avgY - 6);
          ...
          const getX = (index: number) => paddingLeft + (index / (pointsCount - 1)) * chartWidth;
          const getY = (value: number) => h - paddingBottom - (value / 18) * chartHeight;
          ...
          for (let i = 0; i < pointsCount; i++) {
            const pt = windData[i];
            const px = getX(i);
            const val = pt.y + Math.sin(offset + i * 0.5) * 0.2;
            const py = getY(val);
            ctx.lineTo(px, py);
          }
          ctx.lineTo(getX(pointsCount - 1), h - paddingBottom);
          ctx.closePath();
          ctx.fill();
          ...
          for (let i = 0; i < pointsCount; i++) {
            ...
            if (i % 2 === 0) {
              ...
              ctx.fillText(`${val.toFixed(1)}`, px, py - 10);
            }
          }
          ...
          ctx.textAlign = 'right';
          for (let val = 0; val <= 18; val += 6) {
            ctx.fillText(`${val} m/s`, paddingLeft - 8, getY(val) + 3);
          }

          // Bottom X axis labels (hours)
          ctx.textAlign = 'center';
          const nowHour = new Date().getHours();
          for (let i = 0; i < pointsCount; i += 2) {
            const hLabel = (nowHour - (pointsCount - 1 - i) * 2 + 24) % 24;
            ctx.fillText(`${String(hLabel).padStart(2, '0')}:00`, getX(i), h - 6);
          }
    ...
    try {
      ...
      const searchPromises = allWarehouses.map(async (wh) => {
        try {
          ...
          console.error(`Failed to search global stock for warehouse ${wh.id}:`, e);
        }
        return null;
      });
  ...
  return `
    <div class="fade-in-up dashboard-container">
      ...
      <div class="dash-stats-grid">
        ...
        <div class="dash-stat-card primary">
          <div style="flex: 1; display: flex; flex-direction: column; gap: 4px;">
            <span class="label">İŞ TAMAMLAMA</span>
            <span class="value" data-count="${completionRate}">${completionRate}%</span>
            <span class="sub-label">Bugünkü İşlerin Bitirilme Oranı</span>
          </div>
          ${renderCircularGauge(completionRate, '#00f3ff', '#4facfe', 'completion')}
        ...
        <div class="dash-stat-card danger">
          <div style="flex: 1; display: flex; flex-direction: column; gap: 4px;">
            <span class="label">ARIZA ORANI</span>
            <span class="value" data-count="${faultRate}">${faultRate}%</span>
            <span class="sub-label">Aktif Görevler Arasındaki Arıza Oranı</span>
          </div>
          ${renderCircularGauge(faultRate, '#ff4d4d', '#f857a6', 'fault')}
          <div class="card-glow danger"></div>
        </div>

        <!-- CARD 3: Critical Maintenances -->
        <div class="dash-stat-card info" onclick="window.navigate('bakim-planlama')">
          <div style="flex: 1; display: flex; flex-direction: column; gap: 4px;">
            <span class="label">KRİTİK BAKIMLAR</span>
            <span class="value" data-count="${maintenanceRatio}">${maintenanceRatio}%</span>
            <span class="sub-label">Yaklaşan veya Geciken Bakımlar</span>
          </div>
          ${renderCircularGauge(maintenanceRatio, '#a29bfe', '#6c5ce7', 'maintenance')}
          <div class="card-glow info"></div>
        </div>

        <!-- CARD 4: Turbine Health -->
        <div class="dash-stat-card warning">
          <div style="flex: 1; display: flex; flex-direction: column; gap: 4px;">
            <span class="label">TÜRBİN SAĞLIĞI</span>
            <span class="value" data-count="${healthRate}">${healthRate}%</span>
            <span class="sub-label">Faal / Arızasız Türbin Oranı</span>
          </div>
          ${renderCircularGauge(healthRate, '#10b981', '#059669', 'health')}
      ...
      <div class="dash-agenda-row">
        ...
        <!-- RIGHT SIDE CONTAINER WITH WIND SPEED & GRID -->
        <div style="display: flex; flex-direction: column; gap: 1.5rem; flex: 1;">
          <!-- 🌬️ WIND SPEED TELEMETRY PANEL -->
          <div class="glass-panel" style="padding: 1.5rem; position: relative; overflow: hidden; border-top: 2px solid var(--accent-cyan);">
            <div class="section-header" style="margin-bottom: 1rem;">
              <h3 style="font-family: 'Rajdhani', sans-serif; font-size: 1rem; color: var(--text-main); letter-spacing: 1px; margin: 0; display: flex; align-items: center; gap: 8px;">
                <i class="fa-solid fa-wind" style="color: var(--accent-cyan); animation: spin-slow 8s linear infinite;"></i> RÜZGAR HIZI CANLI TELEMETRİ (SAHA GENELİ)
              </h3>
              <span class="count-tag" style="background: rgba(0, 243, 255, 0.1); color: var(--accent-cyan); border: 1px solid rgba(0, 243, 255, 0.3);">GERÇEK ZAMANLI</span>
            </div>
            <canvas id="wind-speed-canvas" style="width: 100%; height: 160px; display: block; background: rgba(0,0,0,0.15); border-radius: 12px; border: 1px solid rgba(255,255,255,0.03);"></canvas>
          </div>

          <div class="dash-agenda-right" style="display: grid; grid-template-columns: 1fr 350px; gap: 1.5rem;">
            <!-- Live Activity Feed -->
            <div class="glass-panel dash-feed-section" style="margin: 0;">
              <div class="section-header">
                <h3><i class="fa-solid fa-person-running"></i> AKTİF GÖREV AKIŞI</h3>
                <span class="count-tag">${activeTasksCount} GÖREV</span>
              </div>
              <div class="feed-container custom-scrollbar" style="max-height: 380px;">
            ...
            <div class="dash-sidebar-section">
              <div class="glass-panel stock-search-card">
                ...
                <div id="global-stock-results" class="results-container custom-scrollbar" style="max-height: 180px;">
    ...
    <style>
      ...
      .dash-stat-card { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); border-radius: 18px; padding: 1.2rem; display: flex; align-items: center; justify-content: space-between; gap: 1rem; position: relative; overflow: hidden; transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); cursor: pointer; }
      ...
      .dash-stat-card .label { font-size: 0.68rem; font-weight: 800; color: rgba(255,255,255,0.4); letter-spacing: 1.5px; }
      .dash-stat-card .value { font-size: 2.1rem; font-weight: 900; color: #fff; font-family: 'Rajdhani', sans-serif; line-height: 1.1; margin: 2px 0; }
      ...
      body.light-mode .dash-stat-card .label {
        color: #64748b;
      }
      body.light-mode .dash-stat-card .value {
        color: #0f172a;
      }
      body.light-mode .dash-stat-card .sub-label {
        color: #64748b;
      }
      body.light-mode .agent-mini-tag {
        background: #f1f5f9;
      ...
      body.light-mode .feed-item .site-tag {
        color: #0f172a;
      }
      body.light-mode .feed-item .personnel-row {
        color: #334155;
      }
      body.light-mode .feed-item .task-desc-row {
        background: #f1f5f9;
        color: #475569;
      }
      ...
      body.light-mode .stock-result-item {
        background: #ffffff;
        border: 1px solid #cbd5e1;
      }
      body.light-mode .stock-result-item .site {
        color: #0f172a;
      }
      body.light-mode .plan-table th {
      ...
      body.light-mode .agenda-event-item {
        border-bottom: 1px solid #e2e8f0;
      }
      body.light-mode .agenda-event-item:hover {
        background: #f1f5f9;
      }
      body.light-mode .agenda-event-date {
        color: #64748b;
      }
      body.light-mode .agenda-event-line::before {
        background: #e2e8f0;
      }
      body.light-mode .agenda-legend-item {
        color: #64748b;
      }
    </style>
  `;

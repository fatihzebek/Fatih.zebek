              ...
              <div style="display: grid; grid-template-columns: 1fr 220px; gap: 4rem; align-items: start;">
                ...
                <div style="display: flex; flex-direction: column; gap: 1.8rem;">
                  ...
                  <div>
                    ...
                    <input type="text" id="modal-sap-no" required placeholder="Örn: 1002345" 
                  ...
                  <!-- Description -->
                  <div>
                    <label style="display: block; color: #64ffda; font-size: 0.8rem; font-weight: 700; text-transform: uppercase; margin-bottom: 12px; letter-spacing: 0.8px;">AÇIKLAMA / ÜRÜN ADI</label>
      ...
      <div class="zoom-tablet" style="max-width: 1600px; margin: 0 auto; padding: 1.5rem; animation: fadeIn 0.4s ease-out;">
        ...
        <div class="stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 1.2rem; margin-bottom: 2rem;">
          <div class="stats-card">
            ...
            <div class="stats-value">${h.length}</div>
          </div>
          <div class="stats-card" style="border-left: 3px solid var(--danger);">
            <div class="stats-label" style="color: var(--danger);">Kritik Stok</div>
            <div class="stats-value" style="color: var(--danger);">${h.filter(e=>e.criticalLimit&&e.criticalLimit>0&&e.quantity<=e.criticalLimit).length}</div>
          </div>
          <div class="stats-card">
          ...
          <div class="stats-card">
            ...
            <div class="stats-value" style="font-size: 1.2rem;">${g.length>0?x(g[0].timestamp).split(` `)[1]:`-`}</div>
          </div>
        ...
        <div style="display: flex; overflow-x: auto; max-width: 100%; -webkit-overflow-scrolling: touch; background: rgba(255,255,255,0.03); padding: 4px; border-radius: 12px; margin-bottom: 2rem; border: 1px solid rgba(255,255,255,0.05); gap: 4px;">
          <button onclick="window.changeTab('${e}', 'inventory')" class="premium-tab ${s===`inventory`?`active`:``}">
            <i class="fa-solid fa-layer-group"></i> ENVANTER
          </button>
          <button onclick="window.changeTab('${e}', 'history')" class="premium-tab ${s===`history`?`active`:``}">
            <i class="fa-solid fa-history"></i> GEÇMİŞ
          </button>
          <button onclick="window.changeTab('${e}', 'analytics')" class="premium-tab ${s===`analytics`?`active`:``}">
            <i class="fa-solid fa-chart-line"></i> ANALİZ
          </button>
          <button onclick="window.changeTab('${e}', 'audit')" class="premium-tab ${s===`audit`?`active`:``}">
            <i class="fa-solid fa-clipboard-check"></i> SAYIM
          </button>
          <button onclick="window.changeTab('${e}', 'audit_history')" class="premium-tab ${s===`audit_history`?`active`:``}">
            ...
            <div class="ai-banner" style="margin-top: 1rem;">
              <div style="display: flex; align-items: center; gap: 1.5rem;">
                ...
                <div>
                  ...
                  <p style="margin: 4px 0 0 0; color: var(--text-dim); font-size: 0.85rem;">Şu an ${h.filter(e=>e.criticalLimit&&e.criticalLimit>0&&e.quantity<=e.criticalLimit).length} kalem malzeme kritik seviyenin altında. Sarf malzemeleri için sipariş planlanması önerilir.</p>
                </div>
              </div>
                ...
                <table style="min-width: 980px; width: 100%; border-collapse: collapse;">
                  <thead>
                    <tr style="background: rgba(255,255,255,0.02); border-bottom: 1px solid rgba(255,255,255,0.05);">
                      ...
                      <th style="padding: 1.2rem 1.5rem; text-align: left; font-size: 0.65rem; color: ${window.lastSortBy===`description`?`var(--accent-blue)`:`var(--text-dim)`}; text-transform: uppercase; letter-spacing: 1px; cursor: pointer;" onclick="window.updateWarehouseUI('${e}', 'description')">
                          MALZEME TANIMI <i class="fa-solid fa-sort${window.lastSortBy===`description`?window.lastSortDir===`asc`?`-up`:`-down`:``}" style="margin-left: 5px; opacity: ${window.lastSortBy===`description`?`1`:`0.5`};"></i>
                      </th>
                      <th style="padding: 1.2rem 1.5rem; text-align: center; font-size: 0.65rem; color: ${window.lastSortBy===`sapNo`?`var(--accent-blue)`:`var(--text-dim)`}; text-transform: uppercase; letter-spacing: 1px; cursor: pointer;" onclick="window.updateWarehouseUI('${e}', 'sapNo')">
                          SAP NO <i class="fa-solid fa-sort${window.lastSortBy===`sapNo`?window.lastSortDir===`asc`?`-up`:`-down`:``}" style="margin-left: 5px; opacity: ${window.lastSortBy===`sapNo`?`1`:`0.5`};"></i>
                      </th>
                      <th style="padding: 1.2rem 1.5rem; text-align: center; font-size: 0.65rem; color: ${window.lastSortBy===`quantity`?`var(--accent-blue)`:`var(--text-dim)`}; text-transform: uppercase; letter-spacing: 1px; cursor: pointer;" onclick="window.updateWarehouseUI('${e}', 'quantity')">
                          STOK <i class="fa-solid fa-sort${window.lastSortBy===`quantity`?window.lastSortDir===`asc`?`-up`:`-down`:``}" style="margin-left: 5px; opacity: ${window.lastSortBy===`quantity`?`1`:`0.5`};"></i>
                      </th>
                      ...
                      <th style="padding: 1.2rem 1.5rem; text-align: left; font-size: 0.65rem; color: ${window.lastSortBy===`shelfNo`?`var(--accent-blue)`:`var(--text-dim)`}; text-transform: uppercase; letter-spacing: 1px; cursor: pointer;" onclick="window.updateWarehouseUI('${e}', 'shelfNo')">
                          KONUM <i class="fa-solid fa-sort${window.lastSortBy===`shelfNo`?window.lastSortDir===`asc`?`-up`:`-down`:``}" style="margin-left: 5px; opacity: ${window.lastSortBy===`shelfNo`?`1`:`0.5`};"></i>
                      </th>
                    ...
                    `:r.map(t=>`
                      <tr class="material-row" style="border-bottom: 1px solid rgba(255,255,255,0.02);">
                        <td style="padding: 1rem; text-align: center;">
                          <input type="checkbox" class="item-checkbox" data-id="${t.id}" data-sap="${t.sapNo}" data-desc="${(t.description||``).replace(/"/g,`&quot;`)}" onchange="window.updateBatchCount()" style="width: 18px; height: 18px; cursor: pointer; accent-color: var(--accent-blue);">
                        </td>
                          ...
                          <div style="display: flex; align-items: center; gap: 1.2rem;">
                            ...
                            <div>
                              <div style="display: flex; align-items: center; gap: 8px;">
                                ...
                                ${(()=>{if(!t.lastAuditDate)return``;let e=t.lastAuditDate.toDate?t.lastAuditDate.toDate():new Date(t.lastAuditDate),n=Math.floor((Date.now()-e.getTime())/(1e3*60*60*24)),r=n<30?`audit-fresh`:n<90?`audit-stale`:`audit-expired`,i=n<30?`GÜNCEL`:n<90?`SAYIM YAKLAŞTI`:`SAYIM GEREKLİ`;return`<span class="audit-badge ${r}" title="Son Sayım: ${x(t.lastAuditDate)}"><i class="fa-solid fa-shield-check"></i> ${i}</span>`})()}
                              </div>
                        ...
                        <td style="padding: 1rem 1.5rem; text-align: center;">
                          <div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
                            <span id="sap-${t.id}" style="background: rgba(100, 255, 218, 0.05); color: var(--accent-blue); padding: 4px 12px; border-radius: 8px; font-size: 0.75rem; font-weight: 800; border: 1px solid rgba(100, 255, 218, 0.1); cursor: pointer;" onclick="window.copyToClipboard('${t.sapNo}')">
                              ${t.sapNo}
                            </span>
                            <button class="action-icon-btn" onclick="window.printQR('${t.id}', '${t.sapNo}', '${t.description.replace(/'/g,`\\'`)}')" style="font-size: 0.7rem; width: 28px; height: 28px; background: none; border: 1px solid rgba(255,255,255,0.1);" title="QR Yazdır">
                        ...
                        <td style="padding: 1rem 1.5rem; text-align: center;">
                          <div style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
                            <div style="font-size: 1.1rem; font-weight: 900; color: ${t.criticalLimit&&t.quantity<=t.criticalLimit?`var(--danger)`:`var(--accent-blue)`};">
                              ${t.quantity}
                              <div style="font-size: 0.55rem; color: var(--text-dim); font-weight: 600; margin-top: 2px;">ADET</div>
                            </div>
                            ${(()=>{let e=window.currentTrends?.[String(t.sapNo).trim()];if(e&&e.forecastDays<30&&t.quantity>0){let t=e.forecastDays<7?`#ff4d4d`:`#ff9800`;return`
                                  <div style="margin-top: 8px; padding: 4px 8px; border-radius: 6px; background: ${t}15; color: ${t}; font-size: 0.6rem; font-weight: 900; border: 1px solid ${t}30;" title="Günlük ortalama tüketim: ${e.avgDaily.toFixed(2)}">
                                    <i class="fa-solid fa-hourglass-half" style="margin-right: 4px;"></i> ~${e.forecastDays} GÜN KALDI
                                  </div>
                                `}return``})()}
                            ${(()=>{let e=(t.reservedQuantity||0)+(_.bySap[String(t.sapNo).trim()]||0);return e>0?`
                                  <div style="margin-top: 8px; padding: 4px 10px; background: rgba(100, 255, 218, 0.1); border: 1px solid rgba(100, 255, 218, 0.2); border-radius: 8px; color: var(--accent-blue); font-size: 0.7rem; font-weight: 900; display: flex; flex-direction: column; align-items: center;">
                                    ...
                                    <span style="font-size: 0.9rem;">${t.quantity-e}</span>
                                  </div>
                                `:``})()}
                          </div>
                        </td>
                        <td style="padding: 1rem 1.5rem; text-align: center;">
                          <div onclick="window.showReserveDetails('${String(t.sapNo).trim()}')" style="font-size: 1rem; font-weight: 800; color: #ff9800; cursor: pointer; padding: 10px; border-radius: 12px; transition: all 0.2s;" onmouseover="this.style.background='rgba(255, 152, 0, 0.05)'" onmouseout="this.style.background='none'">
                            ${(()=>{let e=t.reservedQuantity||0,n=_.bySap[String(t.sapNo).trim()]||0;return`
                                <div style="display: flex; flex-direction: column; align-items: center;">
                                  <div style="font-size: 1.1rem;">${e+n}</div>
                                  ${n>0?`<div style="font-size: 0.5rem; opacity: 0.7; color: #ff9800;">(+${n} TASLAK)</div>`:``}
                        ...
                        <td style="padding: 1rem 1.5rem;">
                          <div style="display: flex; align-items: center; gap: 8px; color: var(--text-dim); font-size: 0.8rem; font-weight: 700;">
                            <div class="shelf-dot" style="background: ${t.shelfNo?`var(--accent-blue)`:`rgba(255,255,255,0.2)`};"></div>
                            ${t.shelfNo||`-`}
                          </div>
                        </td>
                        <td style="padding: 1rem 1.5rem; text-align: right;">
                          <div style="display: flex; gap: 8px; justify-content: flex-end;">
                            <button class="action-icon-btn" onclick="window.requestTransferPrompt('${e}', '${t.id}', '${t.description.replace(/'/g,`\\'`)}', '${String(t.sapNo||``).replace(/'/g,`\\'`)}')" style="color: #64ffda; background: rgba(100, 255, 218, 0.05); border-color: rgba(100, 255, 218, 0.1);" title="Transfer Talep Et">
                              <i class="fa-solid fa-truck-ramp-box"></i>
                            </button>
                            <button class="action-icon-btn" onclick="window.reserveMaterialPrompt('${e}', '${t.id}', '${t.description.replace(/'/g,`\\'`)}', ${t.quantity})" style="color: #ff9800; background: rgba(255, 152, 0, 0.05); border-color: rgba(255, 152, 0, 0.1);" title="Rezerve Et">
                              <i class="fa-solid fa-bookmark"></i>
                            </button>
                            <button class="action-icon-btn" onclick="window.showItemHistory('${t.id}', '${t.description.replace(/'/g,`\\'`)}')" style="color: var(--text-dim); background: rgba(255,255,255,0.05); border-color: rgba(255,255,255,0.1);" title="Malzeme Geçmişi">
                              <i class="fa-solid fa-clock-rotate-left"></i>
                            </button>
                            ${u?`
  <button class="action-icon-btn" onclick="window.editMaterial('${e}', '${t.id}')" style="color: var(--accent-blue); background: rgba(100, 255, 218, 0.05); border-color: rgba(100, 255, 218, 0.1);" title="Düzenle">
                              <i class="fa-solid fa-pen-to-square"></i>
                            </button>
                             `:``}
                             ${d?`
  <button class="action-icon-btn" onclick="window.deleteMaterial('${e}', '${t.id}', '${t.description}', '${t.sapNo}')" style="color: var(--danger); background: rgba(255, 77, 77, 0.05); border-color: rgba(255, 77, 77, 0.1);" title="Sil">
